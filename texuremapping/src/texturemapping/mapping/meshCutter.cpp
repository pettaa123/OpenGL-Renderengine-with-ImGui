...
namespace TextureMapping {
	...
	std::optional<std::pair<std::vector<glm::vec2>, std::vector<int>>> MeshCutter::solveOverlayingTexturesConflicts(std::vector<MappingDataSet>& dataSets) {
		...
		while (clonedDataSets.size() > 0) {
			...
			for (const int triangleId : uniqueTriangleIds) {
				...
				int numberOfUnderlaying = vertexInfos[0].underlayingTexCoords.size();
				...
				int validCount = std::count_if(dominatingTexCoords.begin(), dominatingTexCoords.end(), [&](glm::vec2& v) { return PolygonHelper::isPointInsidePolygon(imagePolygon, v); });
				...
				// CASE: Triangle intersects the image polygon
					...
					int imageCornerIdx = itr - imagePolygon.begin();
					...
					if (isImageCornerInsideTriangle) {
						...
						std::vector<glm::vec2> underlayingImageCornerTexCoords;
					...
					// CASE: Image corner is outside the triangle
						...
						if (intersections.size() == 4) {
							...
							// CASE: All triangle vertices are outside the image polygon
								...
								int trueCount = std::count_if(verticesByRelativePosition.begin(), verticesByRelativePosition.end(), [](const std::pair<VertexInformation, bool>& p) {return p.second == true; });
...
#endif
			std::remove(clonedDataSets.begin(), clonedDataSets.end(), dominatingSet);
			//clonedDataSets.Remove(dominatingSet);

		}

		int vertexCountWithNulls = m_modelVertices.size();
		std::vector<int> modelOriginIds;
		modelOriginIds.reserve(vertexCountWithNulls / 3);
		...
		//	glm::vec3 vertex = m_modelVertices[i];
		//	if (vertex.HasValue) {
		//		for (int a = 0; a < 3; a++) {
		//			int idx = i + a;
		//			m_model.vertices.insert(m_modelVertices[idx].Value);
		//			m_model.normals.insert(m_modelNormals[idx].Value);
		//			modelTexCoordsList.insert(modelTexCoordsList.end(),modelTexCoords[idx].begin(), modelTexCoords[idx].end());
		//		}
	...
	void MeshCutter::handleTriangleReplacements(const std::vector<TriangleReplacement>& triangleReplacements) {
		for (size_t i = 0; i < triangleReplacements.size(); i++) {
			TriangleReplacement replacement = triangleReplacements[i];
			int numberOfNewVertices = replacement.newVertices.size();
	...
	void MeshCutter::handleVertexAndTextureCoordinateReplacement(const TriangleReplacement& replacement) {
		int triangleStartVertexId = replacement.triangleStartVertexId;
		int numberOfNewTriangles = replacement.newVertices.size() / 3;
	...
	std::vector<IntersectionResult> MeshCutter::calculateTriangleIntersections(std::vector<glm::vec2> imagePolygon, std::vector<VertexInformation> vertices) {
		int numberOfUnderlaying = vertices[0].underlayingTexCoords.size();
	...
	VectorResult MeshCutter::determineValidAndInvalidVectors(int triangleStartVertexIdx, const std::vector<glm::vec2>& textureCoordinates, const std::vector<glm::vec2>& imagePolygon) {
		...
		for (auto& c : validTextureCoordinates) {
			...
			validIndices.push_back(itr - textureCoordinates.begin());
		}
		...
		for (auto& c : invalidTextureCoordinates) {
			auto itr = std::find(textureCoordinates.begin(), textureCoordinates.end(), c);
			invalidIndices.push_back(itr - textureCoordinates.begin());
		}

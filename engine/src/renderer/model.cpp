...
namespace Engine {
	...
	Mesh Model::processMesh(void* mesh_in, void* scene_in)
	{
		...
		m_vertexBuffer->setData(vertices.data(), vertices.size() * sizeof(Mesh::Vertex));
		...
		m_indexBuffer->setData(indices.data(), indices.size() * sizeof(Mesh::Index));
		...
		for (auto type : textureTypes) {
			...
			{
				aiString str;
				material->GetTexture(type.first, i, &str);
